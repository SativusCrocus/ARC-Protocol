...
// ── Analysis Types (mirrors backend/data_agent.py ANALYSIS_TYPES) ───────────
...
const ANALYSIS_TYPES = [
  {
    ...
    label: "Trends",
    icon: TrendingUp,
    desc: "Time-series momentum + regime detection + seasonality",
  },
  {
    key: "correlations",
    label: "Correlations",
    icon: LineChart,
    desc: "Pearson / Spearman / Granger causality + lead-lag",
  },
  {
    key: "anomaly_detection",
    label: "Anomaly Detection",
    icon: Activity,
    desc: "Isolation forest + LOF + one-class SVM outlier sweep",
  },
  {
    key: "summary",
    label: "Summary",
    icon: Table,
    desc: "Descriptive stats + missingness + executive one-pager",
  },
] as const;
...
const STEPS = [
  ...
  { key: "profile", label: "Profile", icon: Database, color: "#6366F1" },
  { key: "analyze", label: "Analyze", icon: SlidersHorizontal, color: "#a855f7" },
  { key: "insights", label: "Insights", icon: Sparkles, color: "#38BDF8" },
...
// ── Analyze & Inscribe Button (one-click ARC CLI + verify) ──────────────────

function AnalyzeInscribeButton({
  result,
  onVerified,
}: {
  result: DataResult;
  onVerified: (v: DataVerifyResult) => void;
}) {
  const [copied, setCopied] = useState(false);
  const [verifying, setVerifying] = useState(false);

  const verify = useMutation({
    mutationFn: () => api.dataVerify(result.final_id),
  ...
  return (
    <Button
      ...
      className="bg-[#6366F1]/10 border border-[#6366F1]/30 text-[#6366F1] hover:bg-[#6366F1]/20 data-btn-glow transition-all"
    >
...
export default function DataPage() {
  ...
  const [dataset, setDataset] = useState("arc-mesh-telemetry-stream");
  const [rowsHint, setRowsHint] = useState<number>(100000);
  const [model, setModel] = useState("llama3.1:8b");
  const [result, setResult] = useState<DataResult | null>(null);
  const [verification, setVerification] = useState<DataVerifyResult | null>(
    null,
  );
  ...
  const chainQuery = useQuery({
    queryKey: ["data-chain", result?.final_id],
    queryFn: () => api.dataChain(result!.final_id),
    enabled: !!result?.final_id,
  });

  // Fallback: live arc-data seed chain so the DAG viewer is never empty.
  ...
  const seedDataRecords: RecordWithId[] = useMemo(
    () =>
      (seedRecordsQuery.data || [])
        .filter(
          (r) => (r.record.agent.alias || "").toLowerCase() === "arc-data",
        )
  ...
  const allRecords: RecordWithId[] =
    activeRecords.length > 0 ? activeRecords : seedDataRecords;

  const selectedType = ANALYSIS_TYPES.find((t) => t.key === analysisType);
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      mutation.mutate({
        ...
        dataset: dataset.trim() || "arc-mesh-telemetry-stream",
        rows_hint: rowsHint,
        model,
      });
    },
    [prompt, analysisType, dataset, rowsHint, model, mutation],
  );
  ...
  return (
    <div className="space-y-6 anim-fade-up">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-[48px] font-bold tracking-tighter leading-none">
            <span className="text-[#6366F1]">Data</span>{" "}
            <span className="text-white/90">Analysis</span>
          </h2>
          <p className="text-white/25 text-sm mt-2">
            LangGraph + Ollama analytics agent &middot; profile &rarr; analyze
            &rarr; insights &rarr; report &middot; every analysis anchored to
            the full ARC DAG
          </p>
        </div>
        <Link href="/marketplace#demo">
          <Button
            variant="outline"
            className="gap-2 border-[#6366F1]/20 text-[#6366F1]/80 hover:border-[#6366F1]/40 hover:text-[#6366F1]"
          >
      ...
      {/* Analysis Form */}
      <Card className="border-white/[0.06] bg-[#0a0a0a] data-card-glow">
        <CardContent className="p-4">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {ANALYSIS_TYPES.map((t) => {
                  const Icon = t.icon;
                  const active = t.key === analysisType;
                  return (
                    <button
                      key={t.key}
                      type="button"
                      ...
                      className={`group relative flex flex-col items-start gap-1.5 p-3 rounded-lg border text-left transition-all duration-200 ${
                        active
                          ? "border-[#6366F1]/40 bg-[#6366F1]/[0.06] shadow-[0_0_18px_rgba(99,102,241,0.18)]"
                    ...
                    >
                      ...
                      <div>
                        <div
                          className={`text-[11px] font-semibold tracking-wide ${
                            active ? "text-[#6366F1]" : "text-white/70"
                          }`}
                        >
                          {t.label}
                        </div>
                      ...
                      {active && (
                        <span className="absolute top-2 right-2 h-1.5 w-1.5 rounded-full bg-[#6366F1] shadow-[0_0_6px_rgba(99,102,241,0.9)]" />
                      )}
                    </button>
                  );
                })}
            ...
            <div className="relative">
              <Database className="absolute left-3 top-3.5 h-4 w-4 text-white/20" />
              <textarea
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder="Describe the dataset + what to analyze... (e.g., Analyze the ARC Protocol certified-agent mesh telemetry for momentum trends, cross-agent correlations, and anomaly clusters over the last 30 days.)"
                rows={3}
                className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg pl-10 pr-4 py-3 text-sm text-white/80 placeholder:text-white/15 focus:outline-none focus:border-[#6366F1]/30 focus:ring-1 focus:ring-[#6366F1]/20 transition-all resize-none"
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-2.5 text-xs text-white/60 focus:outline-none focus:border-[#6366F1]/30"
                />
              </div>
              <div>
                <label className="text-[9px] text-white/20 uppercase tracking-wider mb-1 flex items-center gap-1">
                ...
                <input
                  ...
                  value={rowsHint}
                  onChange={(e) =>
                    setRowsHint(Math.max(0, Number(e.target.value) || 0))
                  }
                  className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-2.5 text-xs text-white/60 focus:outline-none focus:border-[#6366F1]/30"
                />
              </div>
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div className="flex items-center gap-3 text-[10px] text-white/20 flex-wrap">
                ...
                <span className="flex items-center gap-1">
                  <Layers className="h-3 w-3" /> 6 inscriptions / analysis
                </span>
                <span className="flex items-center gap-1">
                  <Link2 className="h-3 w-3" /> 7-agent memrefs
                </span>
                <span className="flex items-center gap-1">
              ...
              <Button
                ...
                className="bg-[#6366F1]/10 border border-[#6366F1]/20 text-[#6366F1] hover:bg-[#6366F1]/20 disabled:opacity-30 data-btn-glow"
              >
                {mutation.isPending ? (
                  <>
                    <Loader2 className="h-4 w-4 mr-2 animate-spin" />
      ...
      {mutation.isPending && (
        <div className="anim-fade-up">
          <Card className="border-[#6366F1]/10 bg-[#0a0a0a]">
            <CardContent className="p-6">
              <div className="flex flex-col items-center gap-4">
                <div className="relative">
                  <div className="h-12 w-12 rounded-full border-2 border-[#6366F1]/20 border-t-[#6366F1] animate-spin" />
                  <BarChart className="absolute inset-0 m-auto h-5 w-5 text-[#6366F1]/70" />
                </div>
                <div className="text-center">
                  <p className="text-sm text-white/60">
                    Data agent profiling, analyzing, and anchoring...
                  </p>
                  <p className="text-[10px] text-white/20 mt-1">
                    init &rarr; profile &rarr; analyze &rarr; insights &rarr;
                    report &rarr; inscribe
                  </p>
                </div>
        ...
        <div className="space-y-4 anim-fade-up">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              {
                label: "Type",
                value: selectedType?.label || result.analysis_name,
                icon: FileText,
                color: "#00F0FF",
                isText: true,
              },
          ...
          {/* Analysis Report Preview (primary) */}
          <Card className="border-[#6366F1]/15 bg-[#0a0a0a] overflow-hidden data-card-glow">
            <div className="flex items-center gap-2.5 p-4 border-b border-white/[0.04]">
              ...
              <span className="text-sm font-medium text-[#6366F1]">
                {result.analysis_name || "Analysis Report"}
              </span>
              <Badge
                variant="outline"
                className="text-[8px] text-[#6366F1]/80 border-[#6366F1]/30 px-1.5 uppercase"
              >
              ...
              >
                {result.rows_hint.toLocaleString()} rows
              </Badge>
            ...
            <CardContent className="p-4 pt-4">
              <div className="text-[13px] text-white/75 leading-relaxed whitespace-pre-wrap font-mono rounded-lg p-6 max-h-[600px] overflow-y-auto data-report-paper">
                {result.report}
              </div>
              <div className="mt-3 flex items-center gap-4 text-[10px] text-white/25 font-mono flex-wrap">
                ...
                <span>
                  Rows:{" "}
                  <span className="text-white/50">
                    {result.rows_hint.toLocaleString()}
                  </span>
                </span>
              </div>
            </CardContent>
          ...
          {/* Analyze & Inscribe */}
          <Card className="border-[#6366F1]/15 bg-[#0a0a0a] data-card-glow">
            <CardContent className="p-4">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <BarChart className="h-3.5 w-3.5 text-[#6366F1]/70" />
                    <h4 className="text-xs text-white/40 uppercase tracking-wider font-medium">
                      Analyze &amp; Inscribe on Bitcoin
                    </h4>
                  </div>
                  <p className="text-[11px] text-white/30">
                    Calls the ARC CLI inscription envelope and deep-verifies
                    the full analysis chain + cross-agent memrefs.
                  </p>
                </div>
                <AnalyzeInscribeButton
                  result={result}
          ...
          {/* Profile + Analysis + Insights (collapsible) */}
          <OutputSection
          ...
          />
          <OutputSection
            title="Analysis Results (DAG Walk)"
          ...
          <div>
            <div className="flex items-center gap-2 mb-3 flex-wrap">
              <GitBranch className="h-3.5 w-3.5 text-[#6366F1]/70" />
              <h3 className="text-xs font-medium text-white/30 uppercase tracking-wider">
                Data Analysis Provenance Chain
              </h3>
              {result.dag_memrefs.length > 0 && (
                <Badge
                  variant="outline"
                  className="text-[8px] text-[#6366F1]/80 border-[#6366F1]/25 px-1.5"
                >
            ...
            <div className="h-[420px] border border-[#6366F1]/20 rounded-xl overflow-hidden bg-[#020202] data-dag-glow">
              {allRecords.length > 0 ? (
                <Suspense
                  fallback={
                ...
                <div className="h-full flex items-center justify-center">
                  <div className="inline-block h-6 w-6 animate-spin rounded-full border-2 border-white/10 border-t-[#6366F1]" />
                </div>
              )}
            </div>
          </div>
          ...
          <Card className="border-white/[0.04] bg-[#0a0a0a]">
            <CardContent className="p-4">
              <div className="flex items-center gap-2 mb-3">
                <Link2 className="h-3.5 w-3.5 text-[#6366F1]/70" />
                <h4 className="text-xs text-white/40 uppercase tracking-wider font-medium">
              ...
              <p className="text-[10px] text-white/20 mt-3">
                Every analysis record memrefs the latest head of each certified
      ...
      {/* Recent arc-data Inscriptions (seeded) — shown before submission */}
      {!result && !mutation.isPending && seedDataRecords.length > 0 && (
        <div className="space-y-4 anim-fade-up anim-delay-1">
          <div className="grid grid-cols-3 gap-3">
            {[
              {
                label: "Records",
                value: seedDataRecords.length,
                icon: Layers,
                color: "#6366F1",
              },
              {
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <GitBranch className="h-3.5 w-3.5 text-[#6366F1]/70" />
              <h3 className="text-xs font-medium text-white/30 uppercase tracking-wider">
                Recent Data Analysis Inscriptions
              </h3>
              <Badge
                variant="outline"
                className="text-[8px] text-[#6366F1]/80 border-[#6366F1]/25 px-1.5"
            ...
            <div className="h-[420px] border border-[#6366F1]/20 rounded-xl overflow-hidden bg-[#020202] data-dag-glow">
              ...
              >
                <DAGViewer records={seedDataRecords} />
              </Suspense>
            </div>
          </div>
        </div>
        ...
        <Card className="border-white/[0.04] bg-[#0a0a0a] anim-fade-up anim-delay-2">
          <CardContent className="p-4">
            <h4 className="text-xs text-white/30 uppercase tracking-wider mb-3">
              ARC CLI &middot; Direct Analysis Inscription
            </h4>
            <div className="space-y-2 text-[11px] font-mono">
              <div className="flex items-start gap-2">
                <span className="text-[#6366F1]/70">$</span>
                <code className="text-white/50">
                  cd backend &amp;&amp; python data_agent.py --type trends
                  &quot;30d BTC/USD momentum sweep&quot;
                </code>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-[#6366F1]/70">$</span>
                <code className="text-white/50">
                  python data_agent.py --type anomaly_detection --rows 250000
                  &quot;agent-emission outlier sweep&quot;
                </code>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-[#6366F1]/70">$</span>
